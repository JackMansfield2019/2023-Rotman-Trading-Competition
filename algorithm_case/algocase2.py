    ...
    if net_positions + quantity_offered < api.get(session, 'limits')[0]['net_limit'] and gross_positions + quantity_offered < api.get(session, 'limits')[0]['gross_limit']:
        ...
        if TICKS_PER_PERIOD - tick < LAST_MINUTE_THRESHOLD:   
            if potential_profit > value_of_offer:
                ...
                api.post(session, 'tenders', kwargs={'id': tender_id})
                order_id = submit_order(session, 'RITC', 'MARKET', quantity_offered, 'SELL', None)
            else:
                api.delete(session, 'tenders', kwargs={'id': tender_id})
        else:
            if potential_profit > value_of_offer:
                api.post(session, 'tenders', kwargs={'id': tender_id})
            else:
...
def tender_sell(session, held_tenders, tender_offer):    
    ...
    instant_profit_from_sell = shares_to_sell_instantly * (price_offered - api.get(session, 'limits')[0]['vwap'])
    
    potential_profit = 0
    if (gross_positions - current_positions) + abs(current_position - quantity_offered) < api.get(session, 'limits')[0]['gross_limit']:
...
def main():
    with api.requests.Session() as session:
        ...
        while status == 'ACTIVE':
            status = api.get(session, 'case')['status']
            tender = api.get(session, 'tenders')
            if tender: # must check
                print("tender received")
                action = tender['action']
                if action == 'BUY':
                    decision = tender_buy(session, held_tenders, tender_offer)
                else:
                    decision = tender_sell(session, held_tenders, tender_offer)
                spread_ritc = 100*(api.get(session, 'securities', ticker = ticker)[0]['ask']/api.get(session, 'securities', ticker = ticker)[0]['bid'] - 1)
                size = None
                tender_shares = api.get(session, 'securities', kwargs={'ticker': ticker})['position']
                ...
                if tender_shares < 0:
                    ...
                    if spread_ritc < MAX_SPREAD: #or shares_to_sell_instantly >= shares_to_be_shorted: # less volative
                        asks = ticker_ask(session, ticker)
                        asks = api.get(session, "securities/book", ticker = ticker, limit = ORDER_BOOK_SIZE)['asks']
                        shares_accounted_for = 0
                        ask_index = 0
                    ...
                    if spread_ritc < MAX_SPREAD:
                        ...
                        if sell_price > price_offered:
                            order_id = submit_order(session, ticker, 'MARKET', size, 'SELL', None)
                    else:
                        if tick >= 20:
...
if __name__ == '__main__':
    api.signal.signal(api.signal.SIGNIT, api.signal_handler)
    main()